...
# --- CARGA E TRATAMENTO ---
uploaded_file = st.file_uploader("Suba o CSV do INMET (Dados Reais)", type=["csv"])
dados_finais = None
...
if dados_finais is None:
    st.info("Modo de Simulação Ativo: Utilizando distribuição teórica de Weibull.")
...
with col1:
    fig = go.Figure()
    fig.add_trace(go.Histogram(x=dados_finais, histnorm='probability density', name='Dados', marker_color='#1f77b4'))
...
with col2:
    ...
    fig_weibull.update_layout(title="Curva Teórica de Weibull", xaxis_title="Velocidade (m/s)")
    st.plotly_chart(fig_weibull, use_container_width=True)

# --- RESULTADOS ---
st.header("Cálculo Final de Potência")
potencia_media = 0.5 * rho * area_varredura * (c**3) * gamma(1 + 3/k)
col_a, col_b = st.columns(2)
col_a.metric("Potência Disponível no Vento", f"{potencia_media/1000:.2f} kW")
