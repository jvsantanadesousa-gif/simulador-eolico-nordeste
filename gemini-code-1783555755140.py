...
st.title("Simulador de Potencial Eólico - Nordeste Brasileiro")
st.markdown("---")
...
# O usuário define a velocidade média, o código calcula o 'c' necessário
v_media_desejada = st.sidebar.number_input("Velocidade Média do Vento [m/s]", min_value=1.0, value=8.5, step=0.1)

# Cálculo inverso para encontrar o 'c' correspondente à velocidade média definida
c = v_media_desejada / gamma(1 + 1/k)

rho = st.sidebar.number_input("Densidade do ar (ρ) [kg/m³]", value=1.200, format="%.3f")
...
with col1:
    ...
    fig_hist.update_layout(title="Distribuição de Velocidade (Histograma)", xaxis_title="Velocidade (m/s)", yaxis_title="Densidade", plot_bgcolor='white', template='plotly_white')
    st.plotly_chart(fig_hist, use_container_width=True)

with col2:
    fig_pdf = go.Figure()
    fig_pdf.add_trace(go.Scatter(x=v_teorico, y=pdf, line=dict(color='#E74C3C', width=3)))
    fig_pdf.update_layout(title="Função Densidade de Probabilidade (PDF)", xaxis_title="Velocidade (m/s)", yaxis_title="f(v)", plot_bgcolor='white', template='plotly_white')
...
# Cálculo da potência considerando o 'c' derivado da velocidade média
potencia_media = 0.5 * rho * area_varredura * (c**3) * gamma(1 + 3/k)

c1, c2, c3, c4 = st.columns(4)
c1.metric("Velocidade Média Definida", f"{v_media_desejada:.2f} m/s")
